...
namespace MapStudio.UI
{
    public class AssetViewWindow : DockWindow
    {
        ...
        new public bool IsFocused = false;
        ...
        {
            //Categories
            AddCategory(new AssetCategoryFilter(this, "Favorites"));
            AddCategory(new AssetCategoryFilter(this, "Enemies"));
            AddCategory(new AssetCategoryFilter(this, "Items"));
            AddCategory(new AssetCategoryFilter(this, "NPCs"));
            AddCategory(new AssetCategoryFilter(this, "Objects"));

            Config = AssetConfig.Load();

            /*  var extra = Config.Settings.SelectMany(x => x.Value.Categories).Distinct();
              foreach (var category in extra)
        ...
        private void DrawIconMenuBar()
        {
            ...
            if (ImGui.Button($"  {IconManager.STAR_ICON}   "))
            {
                if (selectedAsset != null) {
                    ...
                    Config.AddToFavorites(selectedAsset);
                }
            }
            ImGui.PopStyleColor();
            ImGui.SameLine();
