...
namespace MapStudio.UI
{
    ...
    public class Workspace : DockSpaceWindow
    {
        ...
        public void SaveFileToModFolder(string modFolder)
        {
            var file = ActiveEditor as IFileFormat;
            string folderName = file.FileInfo.FileName.Split('.')[0];
            string folderPath = $"{modFolder}\\RomFS\\region_common\\level\\{folderName}";

            if (!Directory.Exists(folderPath))
                Directory.CreateDirectory(folderPath);

            SaveFileData(file, $"{folderPath}\\{folderName}.lvb");
        }
